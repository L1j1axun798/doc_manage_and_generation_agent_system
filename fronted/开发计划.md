...
- 业务类型按后端字段建模：
  ...
  - `ProjectMember`: `role, can_manage_folder, can_delete, can_restore`
  - `Folder`: `id, project, parent, name, code, sort_order, is_active, is_system_root`
  - `Document`: `id, project, folder, title, description, current_version, can_download, lock_version, deleted_at, created_at, updated_at`
